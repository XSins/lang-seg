...
from encoding.utils import batch_intersection_union, batch_pix_accuracy
...
class LSegmentationModuleZS(pl.LightningModule):
    def __init__(self, data_path, dataset, batch_size, base_lr, max_epochs, **kwargs):
        ...
        if kwargs["backbone"] in ["clip_resnet101"]:
            FSSDataset.initialize(
                img_size=480, datapath=self.args.datapath, use_original_imgsize=False, imagenet_norm=True
            )
        else:
    ...
    def training_step(self, batch, batch_nb):
        if self.args.finetune_mode:
            ...
            if self.args.benchmark == "pascal" and batch["query_ignore_idx"] is not None:
                query_ignore_idx = torch.cat(
                    [batch["support_ignore_idxs"].squeeze(1), batch["query_ignore_idx"]], dim=0
                )
                area_inter, area_union = Evaluator.classify_prediction(out.argmax(dim=1), target, query_ignore_idx)
            else:
                area_inter, area_union = Evaluator.classify_prediction(out.argmax(dim=1), target)
        ...
        if self.global_rank == 0:
            return_value = self.train_average_meter.write_process(
                batch_nb, self.len_train_dataloader, self.current_epoch, write_batch_idx=50
            )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if self.global_rank == 0:
            return_value = self.val_average_meter.write_process(
                batch_nb, self.len_val_dataloader, self.current_epoch, write_batch_idx=50
            )
            if return_value is not None:
                iou, fb_iou = return_value
    ...
    def validation_epoch_end(self, outs):
        ...
        if self.global_rank == 0:
            Logger.tbd_writer.add_scalars(
                "fewshot_data/data/loss", {"trn_loss": self.fewshot_trn_loss, "val_loss": val_loss}, self.current_epoch
            )
            Logger.tbd_writer.add_scalars(
                "fewshot_data/data/miou", {"trn_miou": self.fewshot_trn_miou, "val_miou": val_miou}, self.current_epoch
            )
            Logger.tbd_writer.add_scalars(
    ...
    def configure_optimizers(self):
        ...
        if self.fixed_encoder:
            ...
            params_list.append({"params": self.net.pretrained.act_postprocess1.parameters(), "lr": self.base_lr})
            params_list.append({"params": self.net.pretrained.act_postprocess2.parameters(), "lr": self.base_lr})
            params_list.append({"params": self.net.pretrained.act_postprocess3.parameters(), "lr": self.base_lr})
            params_list.append({"params": self.net.pretrained.act_postprocess4.parameters(), "lr": self.base_lr})
        else:
            params_list = [
    ...
    def train_dataloader(self):
        if self.args.finetune_mode:
            dataloader = FSSDataset.build_dataloader(
                self.args.benchmark, self.args.bsz, self.args.nworker, self.args.fold, "test", self.args.nshot
            )
        else:
            dataloader = FSSDataset.build_dataloader(
                self.args.benchmark, self.args.bsz, self.args.nworker, self.args.fold, "trn"
            )
    ...
    def val_dataloader(self):
        ...
        if self.args.finetune_mode:
            ...
            dataloader = FSSDataset.build_dataloader(
                self.args.benchmark, self.args.bsz, self.args.nworker, self.args.fold, "val"
            )
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--data_path", type=str, default="", help="path where dataset is stored")
        parser.add_argument(
            "--dataset",
            type=str,
            default="pascal",
        ...
        parser.add_argument("--nshot", type=int, default=0)  # 1
        parser.add_argument("--finetune_mode", action="store_true", default=False, help="whether finetune or not")
